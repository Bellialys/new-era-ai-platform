...
- **Нумерованные** активные документы (`00-*`, `01-*` и далее) живут в корне
  репозитория. Это основной канон проектной документации.
- **`docs/`** используется как слой безномерных операционных документов:
  runbook, SLO, release checklist, privacy/retention notes и похожие рабочие
  материалы. Такие документы регистрируются в `activeDocuments`, если они
  являются текущими правилами или runbook-источниками.
- **`docs/audits/`** используется для датированных исторических отчётов аудита.
  Такие отчёты не регистрируются в `activeDocuments`, если они являются
  snapshot-артефактами, а не текущими правилами проекта.
