...
```text
anonymous_sessions — отдельная таблица (добавлена в v0.6+).
# хранит display_name, avatar_seed, color_seed, last_seen_at и converted_user_id
# tasks.anonymous_session_id (text) совпадает с anonymous_sessions.id (text PK)
```
...
Текущий API `POST /api/vote` принимает поле `responseId` как alias для `model_response_id` в теле запроса. Это техническое исключение из-за backward-compatibility; переименование в `modelResponseId` отслеживается как follow-up. Каноническим именем в БД остаётся `model_response_id`.
...
| `anonymous_sessions` | system | none | `id` (text PK) | manual/TTL | service_role only |
| `audit_log` | system | none | none | manual/retention | service_role SELECT/INSERT only |
| `usage_events` | system (analytics) | `auth.users` (nullable) | `user_id` (nullable) | 90-day retention job | service_role only |
| `team_runs` | account | `tasks`, `auth.users` | `task_id`, `user_id` | delete with account | service_role only |
| `team_run_steps` | inherited from `team_runs` | `team_runs` | `team_run_id` | `ON DELETE CASCADE` | service_role only |
| `code_runs` | account | `auth.users` | `user_id` (nullable) | 30-day retention job | service_role only |
| `leaderboard_snapshots` | system (aggregate) | none | none | permanent | public SELECT; service_role INSERT |
| `artifacts` | account | `tasks`, `auth.users` | `task_id`, `user_id` | delete with account + Storage cleanup | service_role only |
| `model_price_history` | system | none | none | permanent | service_role only |
| `cleanup_log` | system | none | none | 180-day retention | service_role only |
