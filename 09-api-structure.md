...
# POST /api/guest, GET /api/code-models, POST /api/code-compare,
# POST /api/judge, POST /api/code-run и admin audit/usage routes реализованы/в alpha
...
| `/api/judge` | POST | Alpha | Получить AI judge verdict и сохранить `tasks.judge_verdict` best-effort |
| `/api/code-run` | POST | Alpha | Запустить код через внешний runner для авторизованного пользователя |
| `/api/admin/audit` | GET | Alpha | Admin-only чтение audit events |
| `/api/admin/usage` | GET | Alpha | Admin-only usage summary по пользователям |
...
Оценивает 2+ ответа моделей через backend/OpenRouter и возвращает blind verdict.

Правила:

- нужна user session или guest cookie `na_guest`;
- `prompt` минимум 3 символа;
- `responses` содержит минимум 2 непустых ответа;
- при наличии `taskId` вердикт сохраняется в `tasks.judge_verdict`;
- rate limit: 3 req/min для user, 1 req/min для guest.

## POST /api/code-run

Запускает код через внешний Piston runner.

Правила:

- нужен авторизованный Supabase user; guest не допускается;
- пользовательский код не выполняется внутри server-side процесса приложения;
- `language` должен быть из allowlist;
- `code` обязателен и ограничен `CODE_RUN_MAX_CHARS`;
- rate limit: 10 req/min на user UUID.

## GET /api/admin/audit

Admin-only endpoint для последних 50 записей `public.audit_log`.

Правила:

- доступ только через `requireAdmin()`;
- `anon` и `authenticated` не имеют прямого доступа к `audit_log`;
- таблица читается через server-side service role.

## GET /api/admin/usage

Admin-only endpoint для usage summary за текущий UTC-день и последние 7 дней.

Правила:

- доступ только через `requireAdmin()`;
- возвращает максимум 50 пользователей;
- считает только `tasks` с непустым `user_id`.
