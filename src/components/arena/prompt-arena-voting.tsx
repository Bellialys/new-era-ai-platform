...
import type { ArenaApiResponse, ArenaResponseView, ArenaModel } from "@/types/arena";
...
export function PromptArenaVoting() {
    ...
    (async () => {
      try {
        const response = await fetch("/api/compare", {
          ...
          body: JSON.stringify({ prompt: prompt.trim(), modelIds: selectedModelIds, modeSlug: "prompt-arena" }),
        ...
        const data = (await response.json()) as { status: string; taskId?: string | null; responses: ArenaApiResponse[] };
        ...
        setErrorMessage(error instanceof Error ? error.message : "Не удалось получить ответы. Попробуйте ещё раз.");
      } finally {
  ...
  async function handleSelectWinner(responseId: string) {
    ...
    if (!taskId) {
      setVoteMessage("Победитель выбран локально: Supabase persistence сейчас недоступен.");
      return;
    }
    ...
    try {
      const response = await fetch("/api/vote", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ taskId, responseId, voteType: "best", anonymousSessionId: getOrCreateAnonymousSessionId() }),
      });

      if (!response.ok) {
        const errorData = (await response.json()) as { message?: string };
        throw new Error(errorData.message || "Не удалось сохранить выбор победителя.");
      }

      setVoteMessage("Победитель сохранён.");
    } catch (error) {
      console.error("Error saving winner vote:", error);
      setVoteMessage(error instanceof Error ? error.message : "Победитель выбран локально, но сохранить его не удалось.");
    } finally {
      setIsSavingVote(false);
    }
  }
  ...
  function handleReset() {
    ...
    abortControllerRef.current = null;
    setPrompt("");
    setSelectedModelIds(availableModels.length >= MODEL_MIN_SELECT ? getDefaultModelIds(availableModels) : []);
    setTaskId(null);
    setResponses([]);
    setWinnerResponseId(null);
    setVoteMessage(null);
      ...
      <div className="grid gap-3">
        {voteMessage ? (
          <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4 text-sm text-slate-200">
            {isSavingVote ? "Сохраняем выбор..." : voteMessage}
          </div>
        ) : null}
        <ArenaResults
          responses={responses}
          isLoading={isLoading}
          loadingModelNames={availableModels.filter((m) => selectedModelIds.includes(m.id)).map((m) => m.name)}
