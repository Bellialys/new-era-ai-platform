...
# DEC-013 - Единый корневой канон нумерованных документов

## Статус

```text
Accepted
# принято в рамках cleanup plan 44 от 2026-07-02
```

## Решение

Нумерованные активные документы проекта (`00-*`, `01-*`, `37-*`, `38-*` и далее)
должны находиться в корне репозитория. Каталог `docs/` используется для
безномерных операционных документов, а `docs/audits/` — для датированных
исторических отчётов аудита.

## Контекст

Перед cleanup 44 документы `37-env-check-policy.md` и
`38-env-check-implementation.md` жили в `docs/`, тогда как остальные
нумерованные активные документы жили в корне. Это создавало двухдомный канон и
усложняло `document-map.json`.

## Причина

Единый корневой канон проще проверять, искать и поддерживать. `docs/` при этом
остаётся полезным как слой runbook/SLO/release/privacy документов, которые не
являются частью основной числовой последовательности.

## Последствия

- `37-env-check-policy.md` и `38-env-check-implementation.md` перенесены в корень.
- Ссылки на прежние пути 37/38 внутри `docs/` заменены на корневые пути.
- `docs/runbook.md`, `docs/slo.md`, `docs/release-checklist.md` и
  `docs/privacy-retention.md` регистрируются как активные операционные документы.
- Датированные отчёты в `docs/audits/` остаются historical snapshots и не
  регистрируются в `activeDocuments`, если они не являются текущими правилами.
